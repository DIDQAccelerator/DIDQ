...
import operator
import threading from dateutil.relativedelta 
...
def Home():
    ...
    met = pd.read_csv('information.csv', sep='|', usecols=['PROJECT', 'FILE_NAME', 'DATE', 'STATUS'])
    met['DATE'] = pd.to_datetime(met['DATE'])
    filtered_df = met[met['DATE'] > one_month_back]
    filtered_df.sort_values('DATE')
    filtered_df = filtered_df.drop_duplicates(subset=['FILE_NAME'], keep='first')
    data = filtered_df.groupby(['PROJECT', 'STATUS']).size().unstack(fill_value=0)
    fig, ax = plt.subplots(figsize=(4, 4))
    data.plot(kind='bar', ax=ax, width=0.2)
    ax.set_xlabel('Project')
    ax.set_ylabel('Count')
    ax.set_title('Project File Wise Count: Past 1 month', fontsize=10)
...
def file_details():
    ...
    if button == 'VIEW DETAILS':
        ...
        return render_template('ViewDetails.html', details=detail_str, latest=latest_str, project=project, file=file, plot=html, plot2=html2)
